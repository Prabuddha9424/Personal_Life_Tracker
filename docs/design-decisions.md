...
| `--border` | `#2a3350` | Card and input borders |
...
The light theme palette is defined in the design tokens during M0. Tokens are CSS custom properties, switched by a `data-theme` attribute on the root element, so components never hard-code colours.
