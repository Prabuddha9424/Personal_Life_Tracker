...
|---|---|---|---|
| US-003-T1 | Add `REFRESH_TOKEN_TTL_DAYS`, `TRUST_PROXY_HOPS` and `CRON_SECRET` to the validated env, the test env and `.env.example`; use the hop count for `trust proxy` | [M0 Foundation](../plans/01-foundation.md), Task 3, step 3 | To do |
| US-003-T2 | Add the rate-limiter factory and the `auth`, `mail` and `internal` limiters, with a test | [M0 Foundation](../plans/01-foundation.md), Task 3, steps 1-2 | To do |
